...
if ($noping) {Write-warning "When using -noping this script will take a long time to complete" -warning }

set-ipphoneTrustAllCertPolicy

[Collections.Generic.List[Object]]$phones = get-content -path $file | convertfrom-csv
...
for ($i = $first; $i -le $last; $i++)
{
    ...
    if ($noping) {$ping = $true}
    else { write-verbose "Ping $ip" -verbose; $ping = test-ipphoneicmp -ipphone $ip }
    if ($ping)       
    {      
        ...
        if (test-ipphoneweb -ipphone $ip )
        {
            ...
            if($index) 
            {
              if ($index.password -eq $null) { $sipcredential = New-Object System.Management.Automation.PSCredential ($index.username, $defaultpassword ) } else  { $sipcredential = New-Object System.Management.Automation.PSCredential ($index.username, (ConvertTo-SecureString $index.password -asplaintext -force) )  }     
